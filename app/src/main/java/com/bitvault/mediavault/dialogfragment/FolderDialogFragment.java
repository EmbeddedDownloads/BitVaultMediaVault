...
public class FolderDialogFragment extends DialogFragment implements DialogAdapter.OnItemClickListener {
    ...
    /**
     * Get  data from other fragment/activity
     */
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.create_new_directory:
                ...
                break;
            case R.id.cancel:
                GlobalBus.getBus().post(new DialogResponse(Constant.CANCEL));
                getDialog().dismiss();
                break;
            case R.id.ok:
                if (parentDirectory != null && parentDirectory.length() > 0) {
                    if (FileUtils.getParentName(parentDirectory).equalsIgnoreCase(keyName)) {
                        ...
                        ((DirectoryDetailActivity) getActivity()).actionCopyOnDialogResponse(parentDirectory, actionType);
    ...
    /**
     * Open dialog window to create a new directory
     */
                    ...
                    if (directoryName.length() <= 20) {
                        if (FileUtils.getSpecialCharacterValidation(directoryName)) {
                            ...
                            if (parentDirectory.contains(getString(R.string.directory_created))) {
                                ...
                                ((DirectoryDetailActivity) getActivity()).actionCopyOnDialogResponse(FileUtils.getInternalStorage() + "/" +
